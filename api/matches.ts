...
const teamNameMap: { [key: string]: string } = {
  ...
  "Benfica": "벤피카", "FC Porto": "FC 포르투", "Sporting CP": "스포르팅 CP", "SC Braga": "브라가", "Guimaraes": "비토리아 기마랑이스", "Moreirense": "모레이렌세", "Arouca": "아로카", "Suwon Bluewings": "수원삼성", 
  ...
  "Asan Mugunghwa": "아산 무궁화", "Gimhae City": "김해시청", "First Vienna": "퍼스트 비엔나", "Macarthur": "맥아서", "Pogoń Siedlce": "포곤 시에들체", "Tychy 71": "티히", "Cracovia Krakow": "크라코비아", "Birmingham": "버밍엄",
  ...
  "Heilongjiang Lava Spring": "헤이룽장", "Hebei Kungfu": "허베이 쿵푸", "Dongguan United": "둥관 유나이티드", "Chengdu Better City": "청두 룽청", "Cultural Leonesa": "쿨투랄 레오네사", "Şanlıurfaspor": "샨리우르파스포르",
  ...
  "Mardin 1969": "마르딘", "Kahramanmaraş İstiklal Spor": "카흐라만마라쉬", "Sichuan Jiuniu": "쓰촨 지우니우", "Wisla Krakow": "비스와 크라쿠프", "TSV Hartberg": "하르트베르크", "Padova": "파도바",
  ...
  "Adanaspor": "아다나스포르", "Yeni Malatyaspor": "말라티아스포르", "Kırklarelispor": "키르클라렐리스포르", "Ajka": "아이카", "Tiszakecske FC": "티서케치케", "Csakvar": "차크바르",
  ...
  "Wolfsberger AC": "볼프스베르거", "Ried": "리트", "Avellino": "아벨리노", "Zalaegerszegi TE": "잘라에게르세그", "Bandırmaspor": "반디르마스포르", "FC Winterthur": "빈터투어", "Grasshoppers": "그라스호퍼",
  ...
  "Fenerbahçe": "페네르바흐체", "União de Leiria": "우니앙 레이리아", "Górnik Łęczna": "고르니크 렝치나", "Wieczysta Kraków": "비에치스타 크라쿠프", "Stal Rzeszów": "스탈 제슈프", "FC BW Linz": "블라우바이스 린츠",
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const leagueStandingsMap: { [key: string]: any[][] } = {}; // 🔥 2차원 배열로 변경 (여러 조를 모두 저장)
        ...
        if (res && res.response && res.response.length > 0 && res.response[0].league.standings) {
          ...
          leagueStandingsMap[leagueKey] = leagueInfo.standings.map((group: any[]) => {
            ...
            return group.map((t: any) => {
              ...
              return {
                rank: t.rank,
                team: { en: tName, ko: mappedName }, // KOR/ENG 글로벌 대응
    ...
    const filteredMatches = rawFilteredMatches.map((item: any) => {
      ...
      return {
        ...
        odds: matchOdds,
        events: mappedEvents,
        standings: correctStandings // 🔥 찾은 정확한 그룹(조)의 순위표만 화면으로 배송!
      };
    })
    .sort((a: any, b: any) => {
