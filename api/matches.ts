...
// [ScoreLab 전용 리그 한글 명칭 및 필터링 리스트]
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 필터링 및 정렬 로직
    const filteredMatches = (matchData.response || [])
      .filter((item: any) => leagueNameMap[item.league.id] !== undefined) // 80개 리그에 포함된 것만 남김
      .map((item: any) => {
        return {
          id: item.fixture.id,
      ...
      .sort((a: any, b: any) => {
        // 1순위: 진행중인 경기(LIVE)를 맨 위로
        const isALive = !['NS', 'FT'].includes(a.status);
        const isBLive = !['NS', 'FT'].includes(b.status);
        if (isALive && !isBLive) return -1;
        if (!isALive && isBLive) return 1;

        // 2순위: 예정된 경기(NS)를 다음으로
