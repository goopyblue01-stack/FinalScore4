...
function MatchDetail({ match, onBack }: { match: any, onBack: () => void }) {
  // 1순위(검정배경) 전용 스타일 - 사장님 요청: 홈/원정 우세에 따른 색상 포인트
  ...
  // 2~5순위 전용 스타일 - 사장님 요청: 낮은 점수도 검정에 가깝게 가독성 강화
      ...
      <main className="max-w-4xl mx-auto px-3 mt-6">
        ...
        <div className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm mb-6">
          ...
          {match.odds ? (
            ...
            <div className="py-10 text-center text-slate-400 text-xs italic bg-slate-50 rounded-2xl border border-dashed">
              이 경기는 현재 해외 배당 준비 중입니다.
            </div>
          )}
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // [수정] 서버에서 이미 정렬되어 오지만, 한 번 더 정렬 로직 확인
              ...
              <div key={match.id} onClick={() => setSelectedMatch(match)}
                ...
                <div className="p-3 md:p-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase ${isLive ? 'bg-rose-100/50 text-rose-500' : 'bg-[#e8f8f0] text-[#56ad6a]'}`}>
                      {match.league}
                    </span>
                    <span className="text-[10px] font-bold" style={{ color: isLive ? orangeHighlight : darkGrey }}>
                      {displayStatus}
                    </span>
                  </div>
                  <div className="flex items-center justify-center gap-3 mb-3">
                    <div className={`flex-1 text-right text-sm md:text-base truncate ${isHomeLiveWin ? 'font-black text-slate-900' : 'font-semibold text-slate-700'}`}>
                      {match.home}
                    </div>
                    ...
                    <div className={`flex-1 text-left text-sm md:text-base truncate ${isAwayLiveWin ? 'font-black text-slate-900' : 'font-semibold text-slate-700'}`}>
                      {match.away}
                    </div>
                  </div>
