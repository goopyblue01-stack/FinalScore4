...
function MatchDetail({ match, onBack }: { match: any, onBack: () => void }) {
  ...
  const getNormalStyle = (h: number, a: number) => {
    if (h > a) return { h: "text-red-500 font-black", a: "text-slate-700 font-normal" }; // 낮은 쪽 더 진하게
    ...
    <div className="min-h-screen bg-[#f8faff] pb-10">
      ...
      <main className="max-w-4xl mx-auto px-3 mt-6">
        <div className={`rounded-[32px] p-6 mb-6 shadow-sm border ${
          !['NS', 'FT'].includes(match.status) ? 'bg-rose-50/50 border-rose-100' : 'bg-white border-slate-100'
        }`}>
        ...
        <div className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm mb-4">
          <div className="flex items-center gap-2 mb-6 text-[#56ad6a] font-bold"><TrendingUp className="w-5 h-5" /><span>예상 스코어 순위 (Top 3)</span></div>
          <div className="flex flex-col gap-3">
            {topPredictions.map((p) => {
              const isRank1 = p.rank === 1;
              const style = isRank1 ? getRank1Style(p.h, p.a) : getNormalStyle(p.h, p.a);
              return (
                <div key={p.rank} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${isRank1 ? 'bg-slate-900 border-slate-800 scale-[1.02] shadow-md' : 'bg-white border-slate-200 shadow-sm'}`}>
                  <div className="flex items-center gap-3">
                    <span className={`text-[10px] font-black px-2 py-1 rounded-md ${isRank1 ? 'bg-[#56ad6a] text-white' : 'bg-slate-200 text-slate-500'}`}>{p.rank}위</span>
        ...
        <div className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm mb-6">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="flex flex-col items-center p-4 bg-red-50 rounded-2xl border border-red-100"><span className="text-[10px] font-bold text-red-400 mb-1">승</span><div className="flex items-center gap-1"><ChevronDown className="w-4 h-4 text-red-500" strokeWidth={3}/><span className="text-lg font-black text-red-500">{match.odds.home}</span></div></div>
            ...
            <div className="flex flex-col items-center p-4 bg-blue-50 rounded-2xl border border-blue-100"><span className="text-[10px] font-bold text-blue-400 mb-1">패</span><div className="flex items-center gap-1"><ChevronUp className="w-4 h-4 text-blue-600" strokeWidth={3}/><span className="text-lg font-black text-blue-600">{match.odds.away}</span></div></div>
          </div>
