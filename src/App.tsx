...
// 🔥 [위장술용 칼럼 데이터 스토리지 - 오직 데이터 기반 프리뷰 10편 풀세트]
const COLUMNS_DATA = [
  {
    id: 1,
    date: "2026-04-16",
    title: "[UCL 프리뷰] 바이에른 뮌헨 vs 레알 마드리드: '배당 4.50'과 피로도 가중치 분석",
    excerpt: "💡 한줄평: 배당 4.50의 열세, 피로도가 가를 승부.\n\n해외 배당 4.50이 가리키는 원정팀의 위기. ScoredLab 엔진이 분석한 피로도 변수가 승부를 결정지을 전망입니다.",
    content: `바이에른 뮌헨과 레알 마드리드의 챔피언스리그 8강전은 우리 사이트에서 가장 주목받는 '배당 역전' 매치업입니다.

1. 배당 데이터와 승리 확률 분석
현재 원정팀 레알 마드리드의 승리 배당은 4.50까지 치솟아 있습니다. 이는 데이터상 승리 확률이 22% 미만임을 의미하며, 홈팀 뮌헨의 압도적 우세가 지표로 나타나고 있습니다. 뮌헨의 홈 화력 가중치는 최근 5경기 평균 대비 12% 상승한 수치를 기록 중입니다.

2. 피로도 가중치 알고리즘 적용
레알 마드리드는 이번 원정을 앞두고 주말 경기에서 충분한 로테이션을 가동해 6일 이상의 휴식을 취했습니다. 반면 뮌헨은 4일 이하의 짧은 휴식 일정을 소화 중입니다. ScoredLab 엔진은 뮌헨의 체력적 열세를 반영해 수비 조직력 수치를 10% 하향 조정했음에도 불구하고, 여전히 공격 지표에서 뮌헨이 우위를 점하고 있습니다.

3. 최종 예상 스코어
포아송 분포 모델은 뮌헨의 화력이 레알 마드리드의 수비벽을 뚫어낼 것으로 예측합니다. 가장 유력한 스코어는 2:1 혹은 3:1 바이에른 뮌헨의 승리입니다.`
  },
  {
    id: 2,
    date: "2026-04-16",
    title: "[UCL 프리뷰] 아스널 vs 스포르팅 CP: 점유율 지표와 역습 효율의 상관관계",
    excerpt: "💡 한줄평: 압도적 점유율 데이터, 역습에 무너질까.\n\n점유율 70%를 상회하는 홈팀의 지배력. 하지만 데이터는 스포르팅의 날카로운 역습 효율에 주목하고 있습니다.",
    content: `아스널과 스포르팅 CP의 경기는 '점유율의 함정' 데이터가 선명하게 나타나는 매치업입니다.

1. 볼 점유율 및 패스 성공률 분석
홈팀 아스널은 최근 홈 5경기에서 평균 68%의 점유율과 90%에 육박하는 패스 성공률(Passes accurate)을 보여주었습니다. 하지만 주목해야 할 지표는 '오프사이드(Offsides)' 발생 빈도입니다. 공격 라인을 높게 설정하는 아스널의 특성상 경기당 평균 4.2회의 오프사이드가 발생하며 흐름이 끊기는 패턴이 관측됩니다.

2. 박스 안 슈팅 효율성 비교
스포르팅 CP는 점유율이 낮음에도 불구하고 전체 슈팅 대비 '박스 안 슈팅(Shots inside box)' 비율이 65%로 매우 높습니다. 이는 적은 기회에서도 실질적인 득점 찬스를 만들어내는 효율이 극대화되어 있음을 뜻합니다.

3. 최종 예상 스코어
배당 흐름은 아스널의 승리를 가리키고 있으나, ScoredLab 예측 엔진은 1:1 무승부 확률을 2순위로 높게 책정하고 있습니다. 가장 유력한 결과는 아스널의 2:1 신승입니다.`
  },
  {
    id: 3,
    date: "2026-04-16",
    title: "[UEL 프리뷰] 셀타 비고 vs 프라이부르크: 피로도 가중치가 가리키는 방향",
    excerpt: "💡 한줄평: 지친 원정팀, 홈 극강의 데이터를 넘을까.\n\n원정팀의 3일 휴식 스케줄. ScoredLab 엔진의 '휴식일 피로도' 지표가 승부의 핵심 변수로 도출되었습니다.",
...
홈팀의 우세 데이터가 쌓이며 알크마르의 승리 배당이 2.20에서 2.05까지 하락(🔻)했습니다. ScoredLab의 포아송 계산기는 양 팀의 공격 가중치를 반영하여 알크마르의 3:1 또는 2:1 다득점 승리를 예측합니다.`
  },
  {
    ...
    excerpt: "💡 한줄평: 극심한 원정 피로도, 저득점을 가리키다.\n\n장거리 원정과 짧은 휴식. ScoredLab 엔진의 피로도 페널티가 득점력 빈곤으로 이어질 확률을 분석합니다.",
    content: `AEK 아테네와 라요 바예카노의 경기는 철저하게 '피로도'와 '수비 스탯'이 지배할 경기입니다.

1. 원정 페널티와 수비 스탯
라요 바예카노는 4일 이하의 짧은 휴식 후 그리스 원정에 나섭니다. ScoredLab의 피로도 알고리즘은 이를 공격력 10% 하향 변수로 인식합니다. 설상가상으로 홈팀 아테네는 최근 5경기에서 상대의 '유효 슈팅'을 최소화하는 강력한 수비 조직력 데이터를 보여주고 있습니다.
...
득점이 많이 나오지 않을 것이라는 데이터가 모이면서 팽팽했던 승리 배당은 아테네 쪽의 약우세로 서서히 이동하고 있습니다. 예측 엔진은 피로도 변수를 100% 반영하여 1:0 아테네의 신승 또는 1:1 무승부라는 저득점 결과를 내놓았습니다.`
  },
  {
    id: 9,
    date: "2026-04-16",
    title: "[UECL 프리뷰] 피오렌티나 vs 크리스탈 팰리스: 파울 스탯과 세트피스",
...
데이터의 우위가 배당률에 그대로 반영되어 피오렌티나의 승리 배당은 1.80대 정배당으로 굳어지고 있습니다. 팰리스가 체력적으로 버티더라도 후반 막판 집중력 저하가 예상되며, 엔진은 피오렌티나의 2:0 완승을 지목하고 있습니다.`
  },
  {
    id: 10,
    date: "2026-04-16",
    title: "[UECL 프리뷰] 스트라스부르 vs 마인츠: 완벽한 밸런스가 만드는 무승부 지표",
    excerpt: "💡 한줄평: 엇비슷한 폼 데이터, 승부의 균형 유지.\n\n공수 지표에서 완벽에 가까운 밸런스를 보이는 두 팀. 데이터 엔진이 가장 고민하는 예측 매치입니다.",
...
const t = {
  ko: {
    ...
    about: "소개", terms: "이용약관", privacy: "개인정보처리방침", contact: "문의하기",
    ...
    predictionDisclaimer: "* 예상 스코어는 ScoredLab 자체 분석 로직에 기반하며, 경기 시작 전까지 해외 배당 흐름에 따라 실시간으로 변동될 수 있습니다.",
  ...
  en: {
    ...
    about: "About Us", terms: "Terms of Service", privacy: "Privacy Policy", contact: "Contact Us",
    ...
    predictionDisclaimer: "* Expected scores are based on ScoredLab's proprietary logic and may fluctuate in real-time based on global odds trends before kickoff.",
  ...
  return (
    <div className="min-h-screen bg-[#f8faff] pb-10">
      ...
      <main className="max-w-2xl mx-auto px-6 mt-10">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-slate-900 leading-tight mb-8 break-keep">{column.title}</h1>
        <div className="prose prose-slate max-w-none text-slate-700 leading-relaxed whitespace-pre-wrap font-medium">{column.content}</div>
        <button onClick={onBack} className="mt-12 w-full py-4 bg-slate-900 text-white font-bold rounded-2xl shadow-lg">{dict.columnBack}</button>
      </main>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#f8faff] pb-10">
      ...
      <main className="max-w-4xl mx-auto px-3 mt-6">
        <div className={`rounded-[32px] p-6 mb-6 shadow-sm border ${!['NS', 'FT', 'AET', 'PEN', 'PST', 'TBD', 'CANC', 'ABD'].includes(match.status) ? 'bg-rose-50/40 border-rose-100' : 'bg-white border-slate-100'}`}>
          ...
          <div className="flex items-center justify-between gap-2 pt-6">
            ...
            <div className="relative flex items-center justify-center min-w-[80px]">
              ...
              <div className="flex items-center gap-2 text-base md:text-xl">
                {match.status === 'NS' ? <span className="text-slate-300 font-bold">VS</span> : <><span className={homeScoreClass}>{match.scoreHome}</span><span className="text-sm font-bold text-slate-300 lowercase">vs</span><span className={awayScoreClass}>{match.scoreAway}</span></>}
              </div>
            </div>
        ...
        {match.stats && match.stats.possession && (
          <div className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm mb-6 animate-in fade-in duration-500">
            <div className="flex items-center gap-2 mb-6 text-[#9d4edd] font-bold"><BarChart3 className="w-5 h-5" /><span>{dict.statsTitle}</span></div>
            <div className="mb-4">
            ...
            <div className="border-t border-slate-50 pt-4">
              ...
              {(match.stats.yellows?.home > 0 || match.stats.yellows?.away > 0) && <StatBar label={dict.statYellows} home={match.stats.yellows?.home} away={match.stats.yellows?.away} />}
              {(match.stats.reds?.home > 0 || match.stats.reds?.away > 0) && <StatBar label={dict.statReds} home={match.stats.reds?.home} away={match.stats.reds?.away} />}
            </div>
          </div>
        )}
            ...
            <div className="flex justify-between gap-4">
              <div className="flex-1 text-left overflow-hidden">
                {match.lineups.filter((l:any) => l.team.id === match.homeId).map((homeLineup: any, idx: number) => (
                  <div key={idx}>
                    <div className="font-black text-slate-800 mb-1">{homeLineup.formation}</div><div className="text-[10px] font-bold text-slate-300 mb-2 border-b border-slate-100 pb-1 uppercase tracking-wider">{dict.startingXI}</div>
                    <ul className="space-y-1.5 mb-4">{homeLineup.startXI?.map((p: any, i: number) => (<li key={i} className="text-xs text-slate-600 flex items-center gap-2"><span className="text-[10px] font-bold text-slate-400 w-4 shrink-0">{p.player.number}</span><span className="truncate">{p.player.name}</span></li>))}</ul>
                    <div className="text-[10px] font-bold text-slate-300 mb-1 uppercase tracking-wider">{dict.coach}</div><div className="text-xs text-slate-500 font-medium truncate">{homeLineup.coach?.name}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex-1 text-right overflow-hidden">
                {match.lineups.filter((l:any) => l.team.id === match.awayId).map((awayLineup: any, idx: number) => (
                  <div key={idx}>
                    <div className="font-black text-slate-800 mb-1">{awayLineup.formation}</div><div className="text-[10px] font-bold text-slate-300 mb-2 border-b border-slate-100 pb-1 uppercase tracking-wider">{dict.startingXI}</div>
                    <ul className="space-y-1.5 mb-4 flex flex-col items-end">{awayLineup.startXI?.map((p: any, i: number) => (<li key={i} className="text-xs text-slate-600 flex items-center gap-2 flex-row-reverse"><span className="text-[10px] font-bold text-slate-400 w-4 text-right shrink-0">{p.player.number}</span><span className="truncate">{p.player.name}</span></li>))}</ul>
                    <div className="text-[10px] font-bold text-slate-300 mb-1 uppercase tracking-wider">{dict.coach}</div><div className="text-xs text-slate-500 font-medium truncate">{awayLineup.coach?.name}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              {match.events.map((ev: any, idx: number) => (
                <div key={idx} className="flex items-center justify-center">
                  <div className="flex-1 flex items-center justify-end gap-2 text-right">
                    {ev.team === 'home' && (
                      <>{ev.type === 'sub' ? (<div className="flex flex-col items-end text-[11px] font-medium leading-tight gap-0.5"><div className="flex items-center gap-1 text-slate-500">{ev.playerOut} <ArrowLeft className="w-3 h-3 text-red-500" strokeWidth={3} /></div><div className="flex items-center gap-1 text-slate-800">{ev.playerIn} <ArrowRight className="w-3 h-3 text-[#56ad6a]" strokeWidth={3} /></div></div>) : (<span className="font-medium text-slate-700 text-sm">{ev.player}</span>)}{ev.type === 'goal' && <span className="text-base">⚽</span>}{ev.type === 'yellow' && <div className="w-2.5 h-3.5 bg-yellow-400 rounded-[2px] border border-yellow-500 shadow-sm shrink-0"></div>}{ev.type === 'red' && <div className="w-2.5 h-3.5 bg-red-500 rounded-[2px] border border-red-600 shadow-sm shrink-0"></div>}</>
                    )}
                  </div>
                  ...
                  <div className="flex-1 flex items-center justify-start gap-2 text-left">
                    {ev.team === 'away' && (
                      <>{ev.type === 'goal' && <span className="text-base">⚽</span>}{ev.type === 'yellow' && <div className="w-2.5 h-3.5 bg-yellow-400 rounded-[2px] border border-yellow-500 shadow-sm shrink-0"></div>}{ev.type === 'red' && <div className="w-2.5 h-3.5 bg-red-500 rounded-[2px] border border-red-600 shadow-sm shrink-0"></div>}{ev.type === 'sub' ? (<div className="flex flex-col items-start text-[11px] font-medium leading-tight gap-0.5"><div className="flex items-center gap-1 text-slate-500"><ArrowLeft className="w-3 h-3 text-red-500" strokeWidth={3} /> {ev.playerOut}</div><div className="flex items-center gap-1 text-slate-800"><ArrowRight className="w-3 h-3 text-[#56ad6a]" strokeWidth={3} /> {ev.playerIn}</div></div>) : (<span className="font-medium text-slate-700 text-sm">{ev.player}</span>)}</>
                    )}
                  </div>
                </div>
              ))}
            ...
            {topPredictions.map((p: any) => {
              ...
              return (
                <div key={p.rank} className={`flex items-center justify-between p-3.5 rounded-2xl border transition-all ${isRank1 ? 'bg-slate-900 border-slate-800 scale-[1.01] shadow-md' : p.rank <= 3 ? 'bg-white border-slate-200' : 'bg-slate-50/50 border-slate-100 opacity-70'}`}>
                  <div className="flex items-center gap-3"><span className={`text-[9px] font-black px-1.5 py-0.5 rounded ${isRank1 ? 'bg-[#56ad6a] text-white' : 'bg-slate-200 text-slate-500'}`}>{p.rank}{lang === 'ko' ? '위' : ''}</span><div className="flex items-center gap-2 text-base"><span className={style.h}>{p.h}</span><span className={isRank1 ? 'text-slate-600' : 'text-slate-400'}>:</span><span className={style.a}>{p.a}</span></div></div>
          ...
          {history.length > 0 && hProps && dProps && aProps ? (
            <div>
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className={`flex flex-col items-center p-4 rounded-2xl border ${hProps.bg}`}><span className={`text-[10px] font-bold mb-1 ${hProps.label}`}>{dict.winShort}</span><div className="flex items-center gap-1">{hProps.icon === 'down' && <ChevronDown className={`w-4 h-4 ${hProps.text}`} strokeWidth={3}/>}{hProps.icon === 'up' && <ChevronUp className={`w-4 h-4 ${hProps.text}`} strokeWidth={3}/>}<span className={`text-lg font-black ${hProps.text}`}>{hProps.val}</span></div></div>
                <div className={`flex flex-col items-center p-4 rounded-2xl border ${dProps.bg}`}><span className={`text-[10px] font-bold mb-1 ${dProps.label}`}>{dict.drawShort}</span><div className="flex items-center gap-1">{dProps.icon === 'down' && <ChevronDown className={`w-4 h-4 ${dProps.text}`} strokeWidth={3}/>}{dProps.icon === 'up' && <ChevronUp className={`w-4 h-4 ${dProps.text}`} strokeWidth={3}/>}<span className={`text-lg font-black ${dProps.text}`}>{dProps.val}</span></div></div>
                <div className={`flex flex-col items-center p-4 rounded-2xl border ${aProps.bg}`}><span className={`text-[10px] font-bold mb-1 ${aProps.label}`}>{dict.loseShort}</span><div className="flex items-center gap-1">{aProps.icon === 'down' && <ChevronDown className={`w-4 h-4 ${aProps.text}`} strokeWidth={3}/>}{aProps.icon === 'up' && <ChevronUp className={`w-4 h-4 ${aProps.text}`} strokeWidth={3}/>}<span className={`text-lg font-black ${aProps.text}`}>{aProps.val}</span></div></div>
              </div>
              {history.length > 1 && (
                <div className="mt-4 border-t border-slate-100 pt-5 flex flex-col gap-2 relative animate-in fade-in duration-500">
                  <div className="text-[10px] text-slate-500 font-bold mb-2 flex items-center justify-between px-1"><span className="flex items-center gap-1.5"><History className="w-3.5 h-3.5"/> {dict.oddsHistoryTitle}</span><span className="text-slate-400 font-normal text-[9px] bg-slate-100 px-2 py-0.5 rounded-md">{dict.oddsHistoryDesc}</span></div>
                  <div className="flex flex-col gap-1.5">
                     {history.slice(1).map((old: any, idx: number) => {
                       const opacityClass = idx === 0 ? 'opacity-80' : idx === 1 ? 'opacity-60' : 'opacity-40';
                       return (<div key={idx} className={`grid grid-cols-3 gap-3 text-center bg-slate-50 rounded-xl py-2 border border-slate-100/50 ${opacityClass} hover:opacity-100 transition-opacity`}><span className="text-xs font-bold text-slate-500">{old.home}</span><span className="text-xs font-medium text-slate-400">{old.draw}</span><span className="text-xs font-bold text-slate-500">{old.away}</span></div>);
                     })}
                  </div>
                </div>
              )}
          ...
          <div className="bg-white rounded-[24px] p-6 border border-slate-100 shadow-sm mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-center whitespace-nowrap">
                <thead><tr className="text-slate-400 border-b border-slate-100"><th className="py-2.5 font-medium w-10">{dict.rank}</th><th className="py-2.5 font-medium text-left">{dict.team}</th><th className="py-2.5 font-medium w-8">{dict.played}</th><th className="py-2.5 font-medium w-8">{dict.win}</th><th className="py-2.5 font-medium w-8">{dict.draw}</th><th className="py-2.5 font-medium w-8">{dict.lose}</th><th className="py-2.5 font-medium w-8">{dict.goalDiff}</th><th className="py-2.5 font-black w-10 text-[#56ad6a]">{dict.points}</th></tr></thead>
                <tbody>
                  {match.standings.map((s: any) => {
                    const isTargetTeam = s.team.en === match.home.en || s.team.en === match.away.en;
                    return (<tr key={s.rank} className={`border-b border-slate-50 last:border-0 ${isTargetTeam ? 'bg-slate-50/80 font-bold' : ''}`}><td className="py-3 text-slate-500">{s.rank}</td><td className={`py-3 text-left ${isTargetTeam ? 'text-slate-900' : 'text-slate-600'}`}>{s.team[lang]}</td><td className="py-3 text-slate-500">{s.played}</td><td className="py-3 text-slate-500">{s.win}</td><td className="py-3 text-slate-500">{s.draw}</td><td className="py-3 text-slate-500">{s.lose}</td><td className="py-3 text-slate-500">{s.goalDiff}</td><td className="py-3 text-[#56ad6a] font-black">{s.points}</td></tr>);
                  })}
                </tbody>
              </table>
            </div>
...
export default function App() {
  const [matches, setMatches] = useState<any[]>([]); const [matchCache, setMatchCache] = useState<{ [key: string]: any[] }>({}); 
  const [selectedDateIdx, setSelectedDateIdx] = useState(2); const [selectedMatch, setSelectedMatch] = useState<any | null>(null); 
  const [selectedPage, setSelectedPage] = useState<string>('main'); const [selectedCol, setSelectedCol] = useState<any | null>(null);
  const [isLoading, setIsLoading] = useState(false); const [isRefreshing, setIsRefreshing] = useState(false);
  const [showBookmarkModal, setShowBookmarkModal] = useState(false); const [showStep2, setShowStep2] = useState(false);
  const [isLiveMode, setIsLiveMode] = useState(false); const [isListView, setIsListView] = useState(false);
  const [lang, setLang] = useState<'ko' | 'en'>('ko'); const dict = t[lang];

  useEffect(() => { const browserLang = navigator.language || (navigator as any).userLanguage; if (browserLang && !browserLang.toLowerCase().includes('ko')) setLang('en'); }, []);
  const today = startOfToday();
  const dates = Array.from({ length: 5 }, (_, i) => { const date = addDays(today, i - 2); return { date, dateStr: format(date, 'yyyy-MM-dd'), dayKo: format(date, 'M월 d일'), dayEn: format(date, 'MMM d'), labelKo: i === 2 ? '(오늘)' : '', labelEn: i === 2 ? '(today)' : '' }; });

  const handleDateClick = (idx: number) => { setSelectedDateIdx(idx); window.history.pushState({}, '', `/?date=${dates[idx].dateStr}`); };
  const handleRefresh = async () => {
    if (isRefreshing) return; setIsRefreshing(true); const dateStr = dates[selectedDateIdx].dateStr;
    try { const res = await fetch(`/api/matches?date=${dateStr}`); const json = await res.json(); const updatedMatches = processMatchesWithTrends(json.matches || [], dateStr); setMatches(updatedMatches); setMatchCache(prev => ({ ...prev, [dateStr]: updatedMatches })); } catch (e) { } finally { setIsRefreshing(false); }
  };
  useEffect(() => {
    const dateStr = dates[selectedDateIdx].dateStr;
    const fetchData = async (isStealthMode = false) => {
      if (!isStealthMode) setIsLoading(true); 
      try { const res = await fetch(`/api/matches?date=${dateStr}`); const json = await res.json(); const updatedMatches = processMatchesWithTrends(json.matches || [], dateStr); setMatches(updatedMatches); setMatchCache(prev => ({ ...prev, [dateStr]: updatedMatches })); } catch (e) { } finally { if (!isStealthMode) setIsLoading(false); }
    };
    if (matchCache[dateStr]) setMatches(matchCache[dateStr]); else fetchData(false);
    let stealthTimer: NodeJS.Timeout; if (selectedDateIdx === 2) { stealthTimer = setInterval(() => { fetchData(true); }, 3 * 60 * 1000); }
    return () => { if (stealthTimer) clearInterval(stealthTimer); };
  }, [selectedDateIdx]);
  useEffect(() => { const handlePopState = () => { setSelectedMatch(null); setSelectedPage('main'); setSelectedCol(null); }; window.addEventListener('popstate', handlePopState); return () => window.removeEventListener('popstate', handlePopState); }, []);
  
  const goToMatch = (match: any) => { setSelectedMatch(match); window.history.pushState({}, '', `/?match=${match.id}`); };
  const goHome = () => { setSelectedMatch(null); setSelectedPage('main'); setSelectedCol(null); setSelectedDateIdx(2); window.history.pushState({}, '', '/'); };
  const goBackToList = () => { setSelectedMatch(null); window.history.pushState({}, '', `/?date=${dates[selectedDateIdx].dateStr}`); };

  if (selectedPage === 'about') return <About onBack={goHome} />; if (selectedPage === 'terms') return <Terms onBack={goHome} />; if (selectedPage === 'privacy') return <Privacy onBack={goHome} />;
  ...
  return (
    <div className="min-h-screen bg-[#f8faff] pb-10 text-slate-900 font-sans tracking-tight">
      <header className="bg-white pt-10 pb-6 flex flex-col items-center border-b border-slate-100 shadow-sm relative">
        <div className="flex flex-col items-center cursor-pointer hover:opacity-80 transition-opacity" onClick={goHome}><h1 className="text-5xl md:text-6xl font-black tracking-tighter italic leading-none"><span className="text-[#0f3460]">Scored</span><span className="text-[#84cc16]">Lab</span></h1><div className="w-16 h-1.5 mt-2 rounded-full" style={{ background: 'linear-gradient(to right, #0f3460, #84cc16)' }}></div></div>
        <div className="w-full max-w-4xl px-4 mt-8 grid grid-cols-5 gap-1.5 md:gap-2 items-center">
          ...
          <button onClick={() => setSelectedPage('columns')} className="h-11 flex flex-row items-center justify-center gap-1.5 rounded-xl font-bold text-xs md:text-sm transition-all shadow-sm border bg-white border-slate-100 text-slate-600 hover:bg-slate-50"><BookOpen className="w-4 h-4 text-[#9d4edd]" />COLUMN</button>
      ...
      {selectedPage === 'columns' ? (
        <main className="max-w-4xl mx-auto px-4 mt-10">
          <h2 className="text-2xl font-black text-slate-800 mb-8 px-2 flex items-center gap-2"><BookOpen className="text-[#9d4edd] w-7 h-7"/> {dict.columnTitle}</h2>
          <div className="grid gap-4 md:grid-cols-2">
            {COLUMNS_DATA.map(col => (
              <div key={col.id} onClick={() => setSelectedCol(col)} className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm cursor-pointer hover:scale-[1.02] transition-transform duration-300">
        ...
        <>
          <nav className="bg-white border-b border-slate-100 py-4 px-4 sticky top-0 z-20 shadow-sm">
            <div className="max-w-4xl mx-auto flex justify-between gap-2">
              {dates.map((date, idx) => (<button key={idx} onClick={() => handleDateClick(idx)} className={`flex-1 h-12 rounded-xl flex flex-col items-center justify-center transition-all ${selectedDateIdx === idx ? 'bg-[#56ad6a] text-white shadow-md' : 'bg-slate-50 text-slate-400'}`}><span className="text-sm font-bold">{lang === 'ko' ? date.dayKo : date.dayEn}</span><span className="text-[9px]">{lang === 'ko' ? date.labelKo : date.labelEn}</span></button>))}
            </div>
          </nav>
          ...
          <main className="max-w-4xl mx-auto px-4 mt-6">
            ...
            {isLoading ? (
              <div className="space-y-3">
                {[1, 2, 3, 4].map((n) => (
                  <div key={n} className="rounded-[24px] border border-slate-100 bg-white p-4 shadow-sm animate-pulse"><div className="flex justify-between items-center mb-4"><div className="w-12 h-3 bg-slate-100 rounded"></div><div className="w-16 h-3 bg-slate-100 rounded"></div></div><div className="flex items-center justify-center gap-3 mb-6 pt-2"><div className="flex-1 h-4 bg-slate-100 rounded"></div><div className="w-8 h-8 bg-slate-100 rounded-full"></div><div className="flex-1 h-4 bg-slate-100 rounded"></div></div></div>
                ))}
              </div>
...
>
  {match.dynamicTitle}
      ...
      {/* 🔥 완전히 새로고침된 신뢰도 100% 푸터 영역! */}
      <footer className="mt-16 py-10 bg-slate-900 border-t border-slate-800 text-center flex flex-col items-center justify-center gap-6">
        <div className="flex flex-wrap justify-center gap-x-5 gap-y-3 text-[11px] md:text-xs text-slate-300 font-medium">
          <button onClick={() => setSelectedPage('about')} className="hover:text-white transition-colors">{dict.about}</button>
          <span className="text-slate-700 hidden sm:inline">|</span>
          <button onClick={() => setSelectedPage('terms')} className="hover:text-white transition-colors">{dict.terms}</button>
          <span className="text-slate-700 hidden sm:inline">|</span>
