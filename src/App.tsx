...
const COLUMNS_DATA = [
  {
    id: 1,
    date: "2026-04-15",
    title: "[분석] 바이에른 뮌헨 vs 레알 마드리드: '배당 4.50' 레알의 위기와 뮌헨의 화력",
    excerpt: "챔피언스리그 8강 최고의 빅매치! ScoredLab의 예측 엔진이 잡아낸 미세한 피로도 변수와 해외 배당 폭락의 이유를 분석합니다.",
    content: `유럽 축구의 최강자, 바이에른 뮌헨과 레알 마드리드의 챔피언스리그 8강 격돌이 다가왔습니다. 이름만 들으면 팽팽한 명승부가 예상되지만, ScoredLab의 실시간 배당 트래커가 포착한 데이터는 전혀 다른 흐름을 가리키고 있습니다.

1. 레알 마드리드 '배당 4.50'이 의미하는 바
현재 우리 웹사이트에 집계된 해외 배당 흐름을 보면, 원정팀 레알 마드리드의 승리 배당이 무려 4.50까지 치솟아 있습니다. 축구 통계에서 배당 4.50은 실질적인 승리 확률이 22% 수준에 불과하다는 뜻입니다. 이는 레알 마드리드의 핵심 자원 이탈과 누적된 원정 피로도를 해외 오즈메이커들이 매우 치명적인 약점으로 평가하고 있다는 확실한 증거입니다.

2. 뮌헨의 압도적 화력과 수비 안정감
바이에른 뮌헨은 알리안츠 아레나 홈구장에서 극강의 폼을 보여주고 있습니다. 특히 김민재와 우파메카노가 버티는 수비진은 안정감을 되찾았고, 전방의 해리 케인을 필두로 한 공격진의 '박스 안 유효 슈팅(Shots inside box)' 비율은 리그 최고 수준을 유지하고 있습니다. ScoredLab의 폼 계산 엔진 역시 뮌헨의 공격 가중치를 매우 높게 산출하고 있습니다.

3. 최종 예측
포아송 분포 확률 모델을 돌려본 결과, 이 경기는 바이에른 뮌헨의 일방적인 흐름이 될 가능성이 높습니다. 레알 마드리드가 끈적한 수비로 전반전을 버티더라도, 후반전 체력 저하 시점에 뮌헨의 파상공세를 막기 어려울 것입니다. ScoredLab은 바이에른 뮌헨의 2:1 혹은 3:1 승리를 예측합니다.`
  },
  {
    id: 2,
    date: "2026-04-15",
    title: "[분석] 아스널 vs 스포르팅 CP: 점유율 70%의 함정과 치명적인 '한 방",
    excerpt: "아스널의 압도적인 패스 성공률 뒤에 숨겨진 오프사이드 리스크. 스포르팅의 반전 가능성을 데이터로 짚어봅니다.",
    content: `잉글랜드 프리미어리그의 강호 아스널과 포르투갈의 자존심 스포르팅 CP의 맞대결입니다. 객관적인 전력에서는 아스널이 앞서지만, 축구는 점유율로만 이기는 스포츠가 아닙니다. ScoredLab의 세부 통계 데이터를 통해 이 경기에 숨겨진 진짜 변수를 파악해 보았습니.

1. 아스널의 압도적 패스 성공률, 하지만...
최근 아스널은 경기당 60~70%의 높은 볼 점유율과 88% 이상의 정교한 패스 성공률(Passes accurate)을 자랑합니다. 하지만 경기를 주도하면서도 공격 라인을 무리하게 끌어올리다 보니, 상대의 빠른 역습에 뒷공간을 내주는 장면이 종종 연출되고 있습니다. ScoredLab 통계에 따르면 아스널의 최근 '피파울(Fouls drawn)' 수치가 증가했는데, 이는 역습을 끊어내기 위한 고의적인 반칙이 늘어났음을 의미합니다.

2. 스포르팅 CP의 극강 '박스 안' 효율성
스포르팅 CP는 강력한 최전방 공격수인 빅토르 요케레스를 필두로, 적은 기회를 확실한 골로 연결하는 팀입니다. 전체 슈팅 대비 '유효 슈팅(Shots on Goal)' 비율이 매우 높으며, 특히 페널티 박스 안에서의 집중력이 뛰어납니다. 아스널이 10번의 중거리 슛을 때릴 때, 스포르팅은 3번의 확실한 박스 안 슈팅으로 경기를 뒤집을 수 있는 능력이 있습니다.

3. 최종 예측
배당률은 아스널의 승리를 가리키고 있지만, 스포르팅의 역습 한 방이 터진다면 경기는 미궁 속으로 빠질 수 있습니다. 아스널이 이른 시간 선제골을 넣지 못한다면 오히려 조급해질 확률이 높습니다. ScoredLab 예측 엔진은 아스널의 2:1 진땀승을 가장 높게 보면서도, 1:1 무승부라는 이변의 가능성(확률 2순위) 역시 절대 무시해서는 안 된다고 경고하고 있습니다.`
  }
];
