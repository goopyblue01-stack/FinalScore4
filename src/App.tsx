...
const COLUMNS_DATA = [
  {
    id: 1,
    date: "2026-04-15",
    title: "[분석] 바이에른 뮌헨 vs 레알 마드리드: 배당 흐름이 말해주는 승부의 향방",
    excerpt: "챔피언스리그 8강 최고의 빅매치! ScoredLab의 예측 엔진이 잡아낸 미세한 피로도 변수와 해외 배당 폭락의 이유를 분석합니다.",
    content: `유럽 축구 최고의 명가들이 격돌하는 바이에른 뮌헨과 레알 마드리드의 챔피언스리그 8강 2차전이 다가오고 있습니다. 이번 경기는 단순한 승부를 넘어, 전술적 유연성과 체력적 한계가 교차하는 지점에서 승부가 갈릴 것으로 보입니다.

1. ScoredLab 예측 엔진의 분석: 피로도 변수
현재 바이에른 뮌헨은 자국 리그 일정으로 인해 주전 선수들의 휴식일이 4일 이하로 매우 빡빡한 상황입니다. ScoredLab의 '휴식일 기반 피로도(Rest Days & Fatigue)' 가중치 로직에 따르면, 뮌헨의 수비 집중력은 경기 후반 15% 이상 하락할 가능성이 큽니다. 반면, 레알 마드리드는 주말 리그 경기에서 대규모 로테이션을 가동하며 6일 이상의 충분한 휴식을 취했습니다. 이는 데이터상으로 레알 마드리드에게 상당한 어드밴티지를 부여합니다.

2. 해외 배당의 움직임과 내포된 의미
초기 배당 발표 이후, 레알 마드리드의 승리 배당이 1.62에서 1.50으로 급락(🔻)하는 추세가 관측되었습니다. 해외 오즈메이커들은 뮌헨의 주축 수비진인 데 리흐트의 미세 부상 소식과 레알 마드리드의 체력적 우위를 즉각 반영한 것으로 보입니다. ScoredLab 히스토리 데이터에 따르면, 이 정도의 배당 하락 폭은 실질적인 승리 확률이 10% 이상 상승했음을 의미합니다.

3. 최종 예측
포아송 분포(Poisson Distribution)를 활용한 득점 확률 계산 결과, 양 팀 모두 득점 가능성이 매우 높지만 체력적 우위를 점한 레알 마드리드가 2:1 혹은 3:1로 승리할 확률이 가장 높은 것으로 나타났습니다. 뮌헨의 강력한 홈 이점이 변수지만, 데이터는 '체력'과 '배당 흐름'의 손을 들어주고 있습니다.`
  },
  {
    id: 2,
    date: "2026-04-15",
    title: "[분석] 아스널 vs 스포르팅 CP: 점유율의 함정과 효율적인 역습의 대결",
    excerpt: "아스널의 압도적인 패스 성공률 뒤에 숨겨진 오프사이드 리스크. 스포르팅의 반전 가능성을 데이터로 짚어봅니다.",
    content: `프리미어리그 선두권을 달리는 아스널과 포르투갈의 복병 스포르팅 CP의 대결입니다. 외관상으로는 아스널의 압도적 우세가 점쳐지지만, ScoredLab의 세부 스탯 데이터는 조금 다른 이야기를 하고 있습니다.

1. 패스 성공률과 오프사이드의 상관관계
최근 5경기 데이터에 따르면, 아스널은 평균 88% 이상의 경이로운 패스 성공률을 기록 중입니다. 하지만 주목해야 할 지표는 '오프사이드 횟수'입니다. 아스널은 공격 라인을 높게 올리는 특성상 경기당 평균 4회 이상의 오프사이드를 범하고 있습니다. 이는 스포르팅 CP의 강력한 오프사이드 트랩 전술에 고전할 수 있음을 암시합니다.

2. 박스 안 슈팅 효율성
ScoredLab의 줄다리기 통계 그래프를 통해 양 팀의 공격 패턴을 분석한 결과, 아스널은 박스 밖 중거리 슈팅 비중이 높은 반면, 스포르팅은 전체 슈팅의 70% 이상을 박스 안(Inside Box)에서 만들어내는 극강의 효율성을 보여줍니다. 이는 점유율은 아스널이 7:3 정도로 압도하더라도, 실질적인 유효 슈팅 숫자는 팽팽할 수 있음을 뜻합니다.

3. 최종 예측
배당 흐름은 현재 정배열을 유지하고 있으나, 아스널의 수비 집중력이 최근 3경기 연속 실점으로 다소 흔들리고 있습니다. ScoredLab 엔진은 1:1 무승부 혹은 아스널의 2:1 신승을 예측합니다. 만약 실시간 배당에서 아스널의 배당이 상승(🔼)하는 흐름이 보인다면, 스포르팅의 반전 가능성을 진지하게 고려해야 할 매치입니다.`
  }
];
