          ...
          <div className="flex items-center justify-between gap-2 pt-6">
            ...
            <div className="relative flex items-center justify-center min-w-[80px]">
              ...
              <div className="flex items-center gap-2 text-base md:text-xl">
                ...
                <><span className={homeScoreClass}>{match.scoreHome}{match.penHome !== null && <span className="text-sm ml-1 text-orange-500">({match.penHome})</span>}</span><span className="text-sm font-bold text-slate-300 lowercase">vs</span><span className={awayScoreClass}>{match.penAway !== null && <span className="text-sm mr-1 text-orange-500">({match.penAway})</span>}{match.scoreAway}</span></>}
              </div>
            </div>
            ...
            <div>
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className={`flex flex-col items-center p-4 rounded-2xl border ${hProps.bg}`}>
                  ...
                  <div className="flex items-center gap-1">{hProps.icon === 'down' && <ChevronDown className={`w-4 h-4 ${hProps.text}`} strokeWidth={3}/>}{hProps.icon === 'up' && <ChevronUp className={`w-4 h-4 ${hProps.text}`} strokeWidth={3}/>}<span className={`text-lg font-black ${hProps.text}`}>{hProps.val}</span></div>
                </div>
                <div className={`flex flex-col items-center p-4 rounded-2xl border ${dProps.bg}`}>
                  ...
                  <div className="flex items-center gap-1">{dProps.icon === 'down' && <ChevronDown className={`w-4 h-4 ${dProps.text}`} strokeWidth={3}/>}{dProps.icon === 'up' && <ChevronUp className={`w-4 h-4 ${dProps.text}`} strokeWidth={3}/>}<span className={`text-lg font-black ${dProps.text}`}>{dProps.val}</span></div>
                </div>
                <div className={`flex flex-col items-center p-4 rounded-2xl border ${aProps.bg}`}>
                  ...
                  <div className="flex items-center gap-1">{aProps.icon === 'down' && <ChevronDown className={`w-4 h-4 ${aProps.text}`} strokeWidth={3}/>}{aProps.icon === 'up' && <ChevronUp className={`w-4 h-4 ${aProps.text}`} strokeWidth={3}/>}<span className={`text-lg font-black ${aProps.text}`}>{aProps.val}</span></div>
                </div>
              </div>
              {history.length > 1 && (
...
export default function App() {
  ...
  const [selectedDateIdx, setSelectedDateIdx] = useState(2); const [selectedMatch, setSelectedMatch] = useState<any | null>(null); const [selectedPage, setSelectedPage] = useState<string>('main');
  ...
  const dates = Array.from({ length: 5 }, (_, i) => {
    const date = addDays(today, i - 2); return { date, dateStr: format(date, 'yyyy-MM-dd'), dayKo: format(date, 'M월 d일'), dayEn: format(date, 'MMM d'), labelKo: i === 2 ? '(오늘)' : '', labelEn: i === 2 ? '(today)' : '' };
  });
  ...
  useEffect(() => { const handlePopState = () => { setSelectedMatch(null); setSelectedPage('main'); }; window.addEventListener('popstate', handlePopState); return () => window.removeEventListener('popstate', handlePopState); }, []);
